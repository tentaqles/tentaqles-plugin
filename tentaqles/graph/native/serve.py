...
from tentaqles.graph.native.security import validate_graph_path, sanitize_label

# Tentaqles embedding service — optional, falls back to keyword matching
_embeddings_available = False
_emb_node_ids: list[str] = []
_emb_matrix = None  # numpy ndarray if loaded
_emb_service = None

def _init_embeddings(graph_path: str) -> None:
    """Try to load pre-computed embeddings and the Tentaqles embedding service."""
    global _embeddings_available, _emb_node_ids, _emb_matrix, _emb_service
    try:
        from tentaqles.embeddings.graphify_hook import load_embeddings
        from tentaqles.embeddings.service import EmbeddingService

        result = load_embeddings(Path(graph_path).parent)
        if result is not None:
            _emb_node_ids, _emb_matrix = result
            _emb_service = EmbeddingService()
            _embeddings_available = True
    except Exception:
        pass  # embeddings not available — keyword fallback
...
def _load_graph(graph_path: str) -> nx.Graph:
    try:
        ...
        print(f"error: graph.json is corrupted ({exc}). Re-run the graph build to rebuild.", file=sys.stderr)
        sys.exit(1)
...
def _score_nodes(G: nx.Graph, terms: list[str], question: str = "") -> list[tuple[float, str]]:
    """Score nodes by relevance. Uses semantic embeddings if available, keyword fallback otherwise."""
    import numpy as np

    # Semantic path: use embeddings if loaded
    if _embeddings_available and question and _emb_service is not None and _emb_matrix is not None:
        try:
            query_emb = _emb_service.embed([question])[0]
            query_norm = query_emb / (np.linalg.norm(query_emb) + 1e-10)
            emb_norms = _emb_matrix / (
                np.linalg.norm(_emb_matrix, axis=1, keepdims=True) + 1e-10
            )
            similarities = emb_norms @ query_norm
            # Return all nodes with similarity > 0.3 (reasonable threshold)
            scored = [
                (float(similarities[i]), _emb_node_ids[i])
                for i in range(len(similarities))
                if similarities[i] > 0.3 and _emb_node_ids[i] in G
            ]
            if scored:
...
def _find_node(G: nx.Graph, label: str) -> list[str]:
    """Return node IDs matching the search term. Semantic search if available, substring fallback."""
    # Try semantic search first
    if _embeddings_available and _emb_service is not None and _emb_matrix is not None:
        try:
            import numpy as np
            query_emb = _emb_service.embed([label])[0]
            query_norm = query_emb / (np.linalg.norm(query_emb) + 1e-10)
            emb_norms = _emb_matrix / (
                np.linalg.norm(_emb_matrix, axis=1, keepdims=True) + 1e-10
            )
...
def serve(graph_path: str = "graphify-out/graph.json") -> None:
    ...
    _init_embeddings(graph_path)
    if _embeddings_available:
        print(f"[tentaqles] Semantic search active ({len(_emb_node_ids)} node embeddings loaded)", file=sys.stderr)
    else:
        print("[tentaqles] Keyword search mode (run embed_graph to enable semantic search)", file=sys.stderr)

    server = Server("tentaqles-graph")
    ...
    def _tool_query_graph(arguments: dict) -> str:
        ...
        scored = _score_nodes(G, terms, question=question)
    ...
    def _tool_god_nodes(arguments: dict) -> str:
        from tentaqles.graph.native.analyze import god_nodes as _god_nodes
    ...
    def _tool_shortest_path(arguments: dict) -> str:
        src_scored = _score_nodes(G, [t.lower() for t in arguments["source"].split()], question=arguments["source"])
        tgt_scored = _score_nodes(G, [t.lower() for t in arguments["target"].split()], question=arguments["target"])
        if not src_scored:
