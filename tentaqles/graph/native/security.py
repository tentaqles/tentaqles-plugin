...
def safe_fetch(url: str, max_bytes: int = _MAX_FETCH_BYTES, timeout: int = 30) -> bytes:
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0 tentaqles/1.0"})
...
def validate_graph_path(path: str | Path, base: Path | None = None) -> Path:
    ...
    *base* defaults to the `graphify-out` directory relative to CWD.
    ...
    trick the system into reading files before any graph has been built.
    ...
    if not base.exists():
        raise ValueError(
            ...
            "Run the graph build pipeline first to build the graph."
        )
