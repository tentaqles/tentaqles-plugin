...
from tentaqles.graph.native.validate import validate_extraction
...
def build_from_json(extraction: dict) -> nx.Graph:
    ...
    if real_errors:
        print(f"[tentaqles] Extraction warning ({len(real_errors)} issues): {real_errors[0]}", file=sys.stderr)
    G = nx.Graph()
