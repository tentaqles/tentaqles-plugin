...
from tentaqles.graph.native.cache import load_cached, save_cached
from tentaqles.graph.native.detect import _IgnoreTree, _is_noise_dir
...
def collect_files(target: Path, *, follow_symlinks: bool = False, root: Path | None = None) -> list[Path]:
    ...
    ignore_tree = _IgnoreTree(root or target)
    if not follow_symlinks:
        results: list[Path] = []
        for ext in sorted(_EXTENSIONS):
            results.extend(
                p for p in target.rglob(f"*{ext}")
                if not any(part.startswith(".") or _is_noise_dir(part) for part in p.relative_to(target).parts)
                and not ignore_tree.is_ignored(p)
            )
    ...
    for dirpath, dirnames, filenames in os.walk(target, followlinks=True):
        ...
        for fname in filenames:
            p = dp / fname
            if p.suffix in _EXTENSIONS and not fname.startswith(".") and not ignore_tree.is_ignored(p):
                results.append(p)
    return sorted(results)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python -m tentaqles.graph.native.extract <file_or_dir> ...", file=sys.stderr)
        sys.exit(1)
