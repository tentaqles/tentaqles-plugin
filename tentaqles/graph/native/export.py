...
from tentaqles.graph.native.security import sanitize_label
from tentaqles.graph.native.analyze import _node_community_map

def _vis_network_inline() -> str:
    vendor = Path(__file__).parent / "_vendor_vis_network.min.js"
    if vendor.exists():
...
def to_html(
    ...
    _vis_js = _vis_network_inline()
    if _vis_js:
        _vis_script_tag = f"<script>{_vis_js}</script>"
    else:
        _vis_script_tag = '<script src="https://unpkg.com/vis-network/standalone/umd/vis-network.min.js"></script>'
...
def to_obsidian(
    ...
    _FTYPE_TAG = {
        "code": "tentaqles/code",
        "document": "tentaqles/document",
        "paper": "tentaqles/paper",
        "image": "tentaqles/image",
    }
    ...
    for node_id, data in G.nodes(data=True):
        ...
        ftype_tag = _FTYPE_TAG.get(ftype, f"tentaqles/{ftype}" if ftype else "tentaqles/document")
        dom_conf = _dominant_confidence(node_id)
        conf_tag = f"tentaqles/{dom_conf}"
    ...
    # Write .obsidian/graph.json to color nodes by community in graph view
    ...
    for idx, cid in enumerate(sorted_cids):
        ...
        for m_idx, node_id in enumerate(sorted_members):
            ...
            canvas_nodes.append({
                "id": f"n_{node_id}",
                "type": "file",
                "file": f"tentaqles/obsidian/{fname}.md",
                "x": nx_x,
                "y": nx_y,
                "width": 180,
                "height": 60,
